...
def get_parser():
    parser = argparse.ArgumentParser(
        description="active site single point mutation scan"
    )
    # parser.parse_args()
    parser.add_argument("-l", "--ligand", help="ligand in PDBQT")
    parser.add_argument("-r", "--receptor", help="receptor in PDB")
    parser.add_argument(
        "-bc", "--boxcfg", help="box config with box_size and box_center"
    )
    parser.add_argument(
        "-mf",
        "--mutation_file",
    ...
    parser.add_argument(
        "-sf", "--scorefile", help="output file name", default="depect_enzde.sc"
    )
...
def vinalocalsearch(receptor, ligand, boxcfg):
    ...
    vinalocalcmd = (
        ...
        + boxcfg
        + " --out "
        + ligand.replace(".pdbqt", "@_" + receptor + "_out.pdbqt")
    )
    AutoDockScoreCmd = (
        "pythonsh "
        + ADT_PATH
        + "compute_AutoDock41_score.py -r "
        + receptor.replace(".pdb", ".pdbqt")
        + " -l "
        + ligand.replace(".pdbqt", "@_" + receptor + "_out.pdbqt")
    )
        ...
        for pAc in pos_list:
            for X in [
                ...
                if pAc[1] != X:
                    ...
                    outputfile(
                        mutant, vinalocalsearch(mutant, ligand, boxcfg), scorefilename
                    )
