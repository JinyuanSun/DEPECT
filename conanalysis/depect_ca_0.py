...
def get_parser():
    ...
    #parser.parse_args()
    parser.add_argument("sequence", help="target sequence which must be provided")
    ...
    parser.add_argument("-nt", '--num_threads', default=4,
                        help='number of threads used to run blastp, default is 4')
    parser.add_argument('mode', help='blast mode require input -ic, -db and -e, analysis mode require -ds')
    #args = parser.parse_args()
    return parser

def build_sub_db(seq,db_name,identity_cutoff,evalue,nt):
    blastout = os.popen("blastp -query "+seq+" -db "+str(db_name)+" -evalue "+str(evalue)+" -outfmt 6 -num_threads "+nt)
...
def cal_score(aa_list):
    ...
    AA_composition={"A":  8.25, "R":  5.53, "N":  4.06, "D":  5.45, "C":  1.37, 
                ...
                "S":  6.56, "T":  5.34, "W":  1.08, "Y":  2.92, "V":  6.87,"X":9.66}
            ...
            if aa != "-":
                ...
                if score > float(score_cutoff):
                    try:
                        ...
                        if aa != raw_aa:
                            out_list.append(raw_target[raw_pos-1]+"\t"+str(raw_pos)+"\t"+aa+"\t"+str(round(score,4)))
                            #out_line = out_line + str(raw_pos) +"\t"+ aa +"\t"+ str(score) + "\n"
                    except KeyError:
                        continue
    return out_list
...
if __name__ == '__main__':
    ...
    if mode == "blast":
        t0 =time.time()
        aln = build_sub_db(seq,db_name,identity_cutoff,evalue,nt)
        print("Blast search finished in",str(time.time()-t0)[0:3]+"s")
        score_dict,map_dict,raw_target = build_matrix(aln,seq)
        outlist = select(score_dict,map_dict,score_cutoff,raw_target)
        with open(seq+".ca","a+") as outf:
